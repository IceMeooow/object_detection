...
The main function to call is Subsample(self, indicator, data). For convenience
one can also call SubsampleWeights(self, weights, data) which is defined in
...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
  ...
  def _get_num_pos_neg_samples(self, sorted_indices_tensor, sample_size):
    ...
    Returns:
      A tuple containing the number of positive and negative data in the
      subsample.
    """
  ...
  def _static_subsample(self, indicator, batch_size, labels):
    ...
    Raises:
      ValueError: if data and indicator are not 1D boolean tensors.
    """
    # Check if indicator and data have a static size.
    ...
    if not labels.shape.is_fully_defined():
      raise ValueError('data must be static in shape when is_static is'
                       'True')
    ...
    # Replace -1 for negative, +1 for positive data
  ...
  def subsample(self, indicator, batch_size, labels, scope=None):
    ...
    if len(labels.get_shape().as_list()) != 1:
      raise ValueError('data must be 1 dimensional, got a tensor of '
                       'shape %s' % labels.get_shape())
    if labels.dtype != tf.bool:
      raise ValueError('data should be of type bool. Received: %s' %
                       labels.dtype)
