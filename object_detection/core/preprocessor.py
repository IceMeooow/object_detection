...
def _strict_random_crop_image(image,
  ...
  with tf.name_scope('RandomCropImage', values=[image, boxes]):
    ...
    new_labels = overlapping_boxlist.get_field('data')
...
def one_hot_encoding(labels, num_classes=None):
  """One-hot encodes the multiclass data.

  Example usage:
    data = tf.constant([1, 4], dtype=tf.int32)
    one_hot = OneHotEncoding(data, num_classes=5)
    one_hot.eval()    # evaluates to [0, 1, 0, 0, 1]

  Args:
    labels: A tensor of shape [None] corresponding to the data.
...
def ssd_random_crop(image,
  ...
  def random_crop_selector(selected_result, index):
    ...
    Args:
      selected_result: A tuple containing image, boxes, data, keypoints (if
                       not None), and masks (if not None).
      index: The index that was randomly selected.

    Returns: A tuple containing image, boxes, data, keypoints (if not None),
