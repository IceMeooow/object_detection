...
def compute_precision_recall(scores, labels, num_gt):
  ...
  Args:
    ...
    labels: A float numpy array representing weighted true/false positive data
  ...
  Returns:
    precision: Fraction of positive instances over detected ones. This value is
      None if no ground truth data are present.
    recall: Fraction of detected positive instance over all positive instances.
      This value is None if no ground truth data are present.

  """
  if not isinstance(labels, np.ndarray) or len(labels.shape) != 1:
    raise ValueError("data must be single dimension numpy array")

  if labels.dtype != np.float and labels.dtype != np.bool:
    raise ValueError("data type must be either bool or float")
  ...
  if len(scores) != len(labels):
    raise ValueError("scores and data must be of the same size.")
...
def compute_median_rank_at_k(tp_fp_list, k):
  """Computes MedianRank@k, where k is the top-scoring data.
...
def compute_recall_at_k(tp_fp_list, num_gt, k):
  """Computes Recall@k, MedianRank@k, where k is the top-scoring data.
